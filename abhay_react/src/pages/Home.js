...
function Home() {
    ...
    try {
      const response = await fetch('http://172.28.206.173:5001/order', {
        method: 'POST',
        headers: {
	...
	navigate('/orderbooked', {state:{id:1,message:data.message,order_selected:JSON.stringify(data.orders)}});  //,{replace:true});
